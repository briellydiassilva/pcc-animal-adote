...
class PaginasPublicasDesignTests(TestCase):
    ...
    def test_dashboard_esta_disponivel_no_menu_e_no_rodape_sem_login(self):
        ...
        self.assertContains(resposta, 'Indicadores do sistema')

    def test_sobre_exibe_equipe_do_projeto_sem_trajetoria_ficticia(self):
        resposta = self.client.get(reverse('sobre'))

        self.assertEqual(resposta.status_code, 200)
        self.assertContains(resposta, 'Lara Vitória Almeida Ferreira')
        self.assertContains(
            resposta,
            'adocao/images/lara-vitoria-almeida-ferreira.png',
        )
        self.assertContains(resposta, 'Gustavo Oliveira Tolentino')
        self.assertContains(
            resposta,
            'adocao/images/gustavo-oliveira-tolentino.png',
        )
        self.assertContains(resposta, 'Gabrielly Dias da Silva')
        self.assertContains(
            resposta,
            'adocao/images/gabrielly-dias-da-silva.png',
        )
        self.assertNotContains(resposta, 'Desenvolvedor(a)')
        self.assertContains(resposta, 'Welinson de Brito dos Santos Batista')
        self.assertContains(
            resposta,
            'adocao/images/welinson-de-brito-dos-santos-batista.png',
        )
        self.assertContains(resposta, 'Professor orientador')
        self.assertNotContains(resposta, 'Espaço reservado')
        self.assertContains(resposta, 'Desenvolvimento do PetAdote')
        self.assertContains(resposta, 'início em novembro de 2025')
        self.assertContains(resposta, 'entrega final de uma plataforma')
        self.assertContains(resposta, 'entre novembro de 2025 e agosto de 2026')
        self.assertNotContains(resposta, 'Nascemos em 2020')
        self.assertNotContains(resposta, 'Nossa Trajetória')
...
class HomeCarrosselTests(TestCase):
    ...
    def test_index_exibe_carrossel_com_animais_e_fotos_do_banco(self):
        ...
        self.assertEqual(resposta.context['total_animais'], 2)
        self.assertEqual(resposta.context['total_adocoes'], 0)
        self.assertContains(resposta, 'Animais cadastrados')
        self.assertContains(resposta, 'Adoções aprovadas')
        self.assertNotContains(resposta, '12.400+')
        self.assertNotContains(resposta, '450+')
        self.assertNotContains(resposta, 'Milhares de cães e gatos')
...
class ContatoTests(TestCase):
    ...
    def test_visitante_envia_mensagem_e_dados_sao_salvos(self):
        ...
        self.assertContains(resposta_confirmacao, 'Mensagem registrada com sucesso')

    def test_contato_nao_exibe_dados_institucionais_ficticios(self):
        resposta = self.client.get(reverse('contato'))

        self.assertContains(resposta, 'Novembro de 2025')
        self.assertContains(resposta, 'Agosto de 2026')
        self.assertNotContains(resposta, '(71) 4002-8922')
        self.assertNotContains(resposta, 'Av. Sete de Setembro')
        self.assertNotContains(resposta, 'contato@petadote.com.br')
